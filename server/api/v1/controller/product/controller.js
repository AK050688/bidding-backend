...
//     // 4. Find highest bid
//     const topBid = await bidschema.findOne({ productId })
//       .sort('-bidAmount')
//       .limit(1);
...
//           totalMRP: { $sum: { $multiply: ["$quantity", "$minBid"] } }
//         }
//       },
//       {
//         $project: {
...
//     const topBrands = await productModel.aggregate(aggregation);
...
//     const totalQuantity = topBrands.reduce((sum, brand) => sum + brand.totalQuantity, 0);
//     const totalMRP = topBrands.reduce((sum, brand) => sum + brand.totalMRP, 0);

//     const formattedResults = topBrands.map(brand => ({
//       brandName: brand.brandName,
//       quantity: `${brand.totalQuantity} (${((brand.totalQuantity / totalQuantity) * 100).toFixed(2)}%)`,
//       lotMRP: `₹ ${brand.totalMRP.toLocaleString('en-IN')} (${((brand.totalMRP / totalMRP) * 100).toFixed(2)}%)`
//     }));
...
//       const product = await productModel.findById(value.productId).populate("sellerId categoryId");
//       console.log(product);
...
//       return res.json(new successResponse(product, responseMessages.SUCCESS));
//     } catch (error) {
